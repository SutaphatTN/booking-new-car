...
//view report insurance (ข้อมูลประกันภัย)
document.addEventListener('DOMContentLoaded', function () {
  const modalEl = document.querySelector('.viewExportInsurance');
  if (!modalEl) return;
...
$('#btnSaveTestDrivePO').on('click', function () {
  ...
  $.ajax({
    ...
    type: 'POST',
    data: testDriveFormData($wrap, { date: $('#po_td_date').val(), note: $('#po_td_note').val() }),
    processData: false,
    contentType: false,
    success: function (res) {
