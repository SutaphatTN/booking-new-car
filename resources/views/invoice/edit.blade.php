...
@section('content')
  ...
  <form id="invoiceForm" action="{{ route('invoice.update', $invoice->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="inv-page-title">
      ...
      <div>
        <h5 class="inv-page-name">แก้ไขใบสั่งซื้อ</h5>
      </div>
    </div>
            ...
            <div class="row g-3 pb-2">
              ...
              <div class="col-md-8">
                ...
                <input id="customer_name" type="text" name="customer_name"
                  class="form-control" value="{{ old('customer_name', $invoice->customer_name) }}" required>
              </div>
              ...
              <div class="col-md-4">
                ...
                <input id="date" type="date" name="date"
                  class="form-control" value="{{ old('date', $invoice->date) }}" required>
              </div>
              ...
              <div class="col-md-5">
                ...
                <input id="customer_phone" type="text" name="customer_phone" maxlength="12"
                  class="form-control" value="{{ old('customer_phone', $invoice->formatted_phone) }}" required>
              </div>
                ...
                <select id="approved_by" name="approved_by" class="form-select" required>
                  ...
                  @foreach ($approvers as $approver)
                    <option value="{{ $approver->id }}"
                      {{ old('approved_by', $invoice->Approved) == $approver->id ? 'selected' : '' }}>
                      {{ $approver->name }}
                    </option>
                  @endforeach
                </select>
            ...
            <div class="row g-3 pb-2">
              ...
              <div class="col-md-4">
                ...
                <input id="license_plate" type="text" name="license_plate" class="form-control"
                  value="{{ old('license_plate', $invoice->license_plate) }}">
              </div>
              ...
              <div class="col-md-8">
                ...
                <input id="engine_number" type="text" name="engine_number" class="form-control"
                  value="{{ old('engine_number', $invoice->engine_number) }}">
              </div>
              ...
              <div class="col-md-8">
                ...
                <input id="vin_number" type="text" name="vin_number" class="form-control"
                  value="{{ old('vin_number', $invoice->vin_number) }}">
              </div>
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0" id="accessoryTable" style="min-width:700px;">
              ...
              <tbody id="accessoryBody">
                @php $accRows = $invoice->accessories->count() ? $invoice->accessories : [null]; @endphp
                @foreach ($accRows as $i => $acc)
                  <tr class="accessory-row">
                    <td style="padding:8px 22px;">
                      @if ($acc)
                        <input type="hidden" name="accessories[{{ $i }}][id]" value="{{ $acc->id }}">
                      @endif
                      <select name="accessories[{{ $i }}][acc_partner]" class="form-select" required>
                        <option value="">-- เลือกร้าน --</option>
                        @foreach ($partners as $p)
                          <option value="{{ $p->id }}" {{ $acc && $acc->acc_partner == $p->id ? 'selected' : '' }}>
                            {{ $p->name }}
                          </option>
                        @endforeach
                      </select>
                    ...
                    <td style="padding:8px 14px;">
                      <input type="text" name="accessories[{{ $i }}][detail]" class="form-control"
                        placeholder="รายละเอียด" value="{{ $acc->detail ?? '' }}" required>
                    </td>
                    <td style="padding:8px 14px;">
                      <input type="text" name="accessories[{{ $i }}][cost_price]" class="form-control money-input text-end"
                        placeholder="0.00" value="{{ $acc && $acc->cost_price !== null ? number_format($acc->cost_price, 2) : '' }}" required>
                    </td>
                    <td style="padding:8px 14px;">
                      <input type="text" name="accessories[{{ $i }}][sale_price]" class="form-control money-input text-end"
                        placeholder="0.00" value="{{ $acc && $acc->sale_price !== null ? number_format($acc->sale_price, 2) : '' }}" required>
                    </td>
